...
parser.add_argument('--model_name', default=r'Baichuan2-7B-Chat',
                    help="Directory containing the dataset")
...
base_model = AutoModelForCausalLM.from_pretrained(args.model_name, quantization_config=bnb_config,trust_remote_code=True)  #原始模型路径
...
with open(args.result_save_dir,"w",encoding='utf-8', errors='ignore') as fw:     #测试结果输出路径
    for i in range(0,len(dataset['output'])):
        ...
        outputs = model.generate(**inputs, max_new_tokens=500)     #最大输出字符长度
        results = tokenizer.decode(outputs[0], skip_special_tokens=True, temperature=0)
        print(results)
        print (str(i)+'+++++++++++results+++++++++++', results)
        fw.write(str(i)+'\t+++++++++++results+++++++++++:'+results.strip()+"\n")
