...
def build_qwen_prompt(input_text, target_label=None):

    system_msg = (
        "You are an expert in NLP. Your task is to extract the relationship between entities. "
        "IMPORTANT: You must ONLY output the relationship label in English. "
        "Do NOT output any Chinese or explanations."
    )
    ...
    if target_label is not None:
        ...
        return f"{prompt}{preprocess(target_label)}<|im_end|>"
...
def evaluate(model, tokenizer, data_path, device, desc="Validation", batch_size=1, output_file=None):
    ...
    sen_file, label_file = os.path.join(data_path, 'sentence'), os.path.join(data_path, 'label')
    eval_data = []
    ...
    with torch.no_grad():
        for i in range(0, len(eval_data), batch_size):
            ...
            prompts = [build_qwen_prompt(b[0]) for b in batch]
    ...
    print(f"\n{desc} Accuracy: {report['accuracy']:.4f}")
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for step in range(0, len(sen_label), args.batch_size):
            ...
            for s, l in batch:
                full_txt = build_qwen_prompt(s, l) + tokenizer.eos_token
                prompt_txt = build_qwen_prompt(s)
...
def test(args):
    ...
    evaluate(model, tokenizer, args.test_path, model.device, desc="Test Set", batch_size=args.batch_size,
