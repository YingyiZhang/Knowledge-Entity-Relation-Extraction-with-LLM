...
def build_llama2_prompt(input_text, target_label=None):
    system_msg = (
        "Classify the relation of the following text:"
    )
    prompt = f"<s>[INST] <<SYS>>\n{system_msg}\n<</SYS>>\n\nText: {preprocess(input_text)}\nLabel: [/INST]"
    if target_label is not None:
        return f"{prompt}{preprocess(target_label)}</s>"
    return prompt
...
def evaluate(model, tokenizer, data_path, device, desc="Validation", batch_size=1, output_file=None):
    ...
    with torch.no_grad():
        for i in range(0, len(eval_data), batch_size):
            ...
            prompts = [build_llama2_prompt(b[0]) for b in batch]
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for step in range(0, len(sen_label), args.batch_size):
            ...
            for s, l in batch:
                full_txt = build_llama2_prompt(s, l) + tokenizer.eos_token
                prompt_txt = build_llama2_prompt(s)
