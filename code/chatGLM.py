...
def build_chatglm_prompt(input_text, target_label=None):
...
def evaluate(model, tokenizer, data_path, device, desc="Validation", batch_size=1, output_file=None):
    ...
    sen_file = os.path.join(data_path, 'sentence_explain')
    label_file = os.path.join(data_path, 'label_prompt')
    ...
    with torch.no_grad():
        for step in range(0, len(eval_data), batch_size):
            ...
            for input_text, target_label in batch:
                ...
                full_prompt = build_chatglm_prompt(clean_input, None)
            ...
            for i, (pred, (orig_input, _)) in enumerate(zip(batch_preds, batch)):

                processed_pred = pred.split("Output:")[
                    -1].strip().lower() if "Output:" in pred else pred.strip().lower()
    ...
    for epoch in range(epochs):
        ...
        for step in range(0, len(sen_label), batch_size):
            ...
            for input_text, target_label in batch:
                ...
                full_prompt = build_chatglm_prompt(clean_input, clean_target)
                input_texts.append(full_prompt)

                input_only_prompt = build_chatglm_prompt(clean_input, None)
                only_input_lens.append(len(tokenizer.encode(input_only_prompt, add_special_tokens=True)))
        ...
        val_loss, val_accuracy = evaluate(model, tokenizer, val_path, device, desc="Validation", batch_size=batch_size, output_file=output_res_path)
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune ChatGLM model with LoRA")

    parser.add_argument("--batch_size", type=int, default=4, help="Batch size for training and evaluation (default: 4)")
    parser.add_argument("--model_path", type=str, required=True, help="Path to ChatGLM model")
    ...
    parser.add_argument("--log_dir", type=str, default="./chatglm_logs", help="Directory to save logs")
    ...
    parser.add_argument("--target_modules", type=str, default="query_key_value", help="Target modules for LoRA")
